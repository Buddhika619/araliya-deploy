...
const ReservationReport = () => {
  ...
  const fetchData = async (id) => {
    try {
      ...
      const { data } = await axios.get(`/api/reports/monthlysales`, config);
  ...
  const totalRevenue = adata.salesItemsArray.reduce(
    (acc, cv) => acc + cv.totalRevenue,
    0
  );
  const totalQty = adata.salesItemsArray.reduce(
    (acc, cv) => acc + cv.quantitySold,
    0
  );
  return (
    <Container>
