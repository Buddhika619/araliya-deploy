...
const getProductListInAdmin = asyncHandler(async (req, res) => {
  ...
  switch (path) {
    case "active":
      products = await Product.find({ active: true, type: true }).sort({
        createdAt: -1,
      });
      break;
...
const getProductListOutAdmin = asyncHandler(async (req, res) => {
  ...
  switch (path) {
    case "active":
      products = await Product.find({ active: true, type: false }).sort({
        createdAt: -1,
      });
      break;
...
const createProduct = asyncHandler(async (req, res) => {
 ...
 const supplier = await Supplier.findById(req.body.supplierId);
  ...
  if (!supplier && req.body.type === false) {
    res.status(400);
  ...
  const product = new Product({
    ...
    reOrderLevel: req.body.reOrderLevel ? req.body.reOrderLevel : 0,
    countInStock: req.body.countInStock ? req.body.countInStock : 0,
...
const updateProduct = asyncHandler(async (req, res) => {
  ...
  if (product) {
    ...
    product.type = type;
    product.supplierId = supplierId && supplierId;
